...
class DatabaseHelperImpl constructor(private val reference: DatabaseReference) : DatabaseHelper {
    ...
    override fun addFavorites(userId: String, stadiums: List<Stadium>) {
        stadiums.forEach { stadium ->
            val userStadium =
                reference.child("users").child(userId).child("stadiums").child(stadium.id)
    ...
    override fun listenToFavoriteStadiums(
        userId: String,
        onFavoriteReceived: (List<Stadium>) -> Unit
    ) {
        reference.child("users").child(userId).child("stadiums")
            .addValueEventListener(object : ValueEventListener {
                override fun onCancelled(error: DatabaseError) {}

                override fun onDataChange(datasnapshot: DataSnapshot) {
                    val values = if (datasnapshot.hasChildren()) datasnapshot.children.map {
                        it.getValue(Stadium::class.java)
                    } else listOf()
    ...
    override fun getFavoriteStadionsOnce(
        userId: String,
        onFavoriteReceived: (List<Stadium>) -> Unit
    ) {
        reference.child("users").child(userId).child("stadiums")
    ...
    override fun getAllStadionsOnce(userId: String, onFavoritesReceived: (List<Stadium>) -> Unit) {
        reference.child("stadiums").addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onDataChange(datasnapshot: DataSnapshot) {
                val values =
                    if (datasnapshot.hasChildren()) datasnapshot.children.map { it.getValue(Stadium::class.java) } else listOf<Stadium>()
    ...
    override fun getUsers(onUsersRecieved: (List<User>) -> Unit) {
        reference.child("users").addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(data: DataSnapshot) {
                val values =
                    if (data.hasChildren()) data.children.map { it.getValue(User::class.java) } else mutableListOf()
